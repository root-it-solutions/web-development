...
if ( ! class_exists( 'Gutenix_Module_Base' ) ) {

	/**
	 * Define Gutenix_Module_Base class
	 */
	abstract class Gutenix_Module_Base {
		 ...
		 *
		 * @param array $childs Childs modules IDs.
		 */
		...
		public function load_child_modules( $childs = array() ) {
			...
			foreach ( $childs as $child => $childs_of_child ) {
				gutenix_theme()->load_module( $child, $childs_of_child );
			}
		 ...
		 * If empty - module will be always initialized
		 *
		 * @return bool
		 */

 ...
 * 2. Add unique prefix for the class name, e.g. - Gutenix_CX_Loader
 * 3. Initialize loader on after_setup_theme hook with priority -20, Example:
 *
 * add_action( 'after_setup_theme', 'gutenix_framework_loader', -20 );
 * function gutenix_framework_loader() {
 *     require get_theme_file_path( 'inc/modules/loader.php' );
 *     new Gutenix_CX_Loader(
 *         array(
 *             get_theme_file_path( 'inc/modules/module-1/module-1.php' ),
 *             get_theme_file_path( 'inc/modules/module-2/module-2.php' ),
 *             get_theme_file_path( 'inc/modules/module-3/module-3.php' ),
 *         )
 *     );
 * }
 *
...
if ( ! class_exists( 'Gutenix_CX_Loader' ) ) {

	/**
	 * Define Gutenix_CX_Loader class
	 */
	class Gutenix_CX_Loader {
			...
			foreach ( $this->modules_slugs as $slug ) {
				...
				if ( file_exists( $path ) ) {
					...
					$this->included_modules[ $slug ] = array(
						...
						'url'  => apply_filters( 'gutenix_cx_include_module_url', trailingslashit( $url ), $path ),
					);

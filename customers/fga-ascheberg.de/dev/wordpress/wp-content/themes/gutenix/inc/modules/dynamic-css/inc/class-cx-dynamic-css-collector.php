...
if ( ! class_exists( 'CX_Dynamic_CSS_Collector' ) ) {
	...
	class CX_Dynamic_CSS_Collector {
		...
		public function get_handle() {
			return apply_filters( 'gutenix_cx_dynamic_css/collector/js_handle', self::$js_handle );
		}
		...
		public function print_style() {
			...
			self::$sorted_css = apply_filters(
				'gutenix_cx_dynamic_css/collector/collected_styles',
				self::$sorted_css
			);
			...
			do_action( 'gutenix_cx_dynamic_css/collector/before_print_collected' );

			array_walk( self::$sorted_css, array( $this, 'print_breakpoint' ) );

			do_action( 'gutenix_cx_dynamic_css/collector/after_print_collected' );

			$styles = ob_get_clean();

			$localize_var = apply_filters( 'gutenix_cx_dynamic_css/collector/localize_object', array(
				'type'  => 'text/css',
		...
		public function print_breakpoint( $rules, $breakpoint ) {
			...
			do_action( 'gutenix_cx_dynamic_css/collector/breakpoint_start', $breakpoint );

			array_walk( $rules, array( $this, 'print_rules' ) );

			do_action( 'gutenix_cx_dynamic_css/collector/breakpoint_end', $breakpoint );
		...
		public function print_rules( $rule, $selector ) {
			...
			do_action( 'gutenix_cx_dynamic_css/collector/rule_start', $selector );

			array_walk( $rule, array( $this, 'print_property' ) );

			do_action( 'gutenix_cx_dynamic_css/collector/rule_end', $selector );

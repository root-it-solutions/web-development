...
class RemapCallbacks
{
    ...
    public function __construct($importer)
    {
        ...
        add_action('zw/import/remap-posts', array($this, 'process_options'));
        add_action('zw/import/remap-posts', array($this, 'postprocess_posts'));
        add_action('zw/import/remap-posts', array($this, 'process_thumbs'));
        add_action('zw/import/remap-posts', array($this, 'process_elementor_pages_posts'));
        add_action('zw/import/remap-posts', array($this, 'process_home_page'));

        // Manipulations with terms remap array
        add_action('zw/import/remap-terms', array($this, 'process_term_parents'));
        add_action('zw/import/remap-terms', array($this, 'process_nav_menu'));
        add_action('zw/import/remap-terms', array($this, 'process_nav_menu_widgets'));
        add_action('zw/import/remap-terms', array($this, 'process_elementor_pages_terms'));
        add_action('zw/import/remap-terms', array($this, 'process_home_page'));
    }
    ...
    public function process_elementor_pages_posts($data)
    {
        ...
        foreach ($pages as $page) {
            ...
            $ids_keys = apply_filters('zw/import/posts/elementor-ids-to-remap', array(
                'panel_template_id',
                'item_template_id',
            ));
    ...
    {
        $pages = $this->elementor_pages();
        $ids_keys = apply_filters('zw/import/terms/elementor-ids-to-remap', array(
            'category_ids',
            'menu',
            'nav_menu',
        ));
    ...
    {
        $regex = apply_filters('zw/import/home-regex-replace', array());

        if (empty($regex)) {
            return false;
        }

        $pages = apply_filters('zw/import/add-pages-to-replace', array());
        if (!empty($pages)) {
    ...
    {
        $todo = \zw\utils\LocalStorage::get('importer.requires_remapping.posts', false);
        $user_slug = \zw\utils\LocalStorage::get('importer.mapping.user_slug', false);
        $url_remap = \zw\utils\LocalStorage::get('importer.url_remap', false);
...
//            }
            ...
            if (in_array(get_post_type($post_id), array('page', 'post'))) {
                ...
                $updated_links = str_replace(
                    \zw\utils\LocalStorage::get('importer.home', ''),
                    home_url(),
                    $old_links);
    ...
    public function process_options($data)
    {
        ...
        // Update ZW site conditions
        $conditions = get_option('zw_site_conditions');
        ...
        if (!empty($conditions)) {
            ...
            update_option('zw_site_conditions', $new_conditions);
        }
    }

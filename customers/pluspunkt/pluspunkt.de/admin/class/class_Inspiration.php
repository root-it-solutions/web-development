...
##############################################################################################################################
	public function getInspirationToCSV($out)
	{
		...
		fputcsv($out, ["Name", "E-Mail", "Anzahl Erwachsene", "Namen Erwachsene", "Anzahl Kinder", "Name Kinnde", "Alter der Kinder", "Feier"], ';');
		...
		if ($inspirtation_result = $GLOBALS["db"]->query($inspirtation_query))
		{
			if ($inspirtation_result->num_rows > 0)
			{
				while ($RowObject = $inspirtation_result->fetch_object())
				{
					fputcsv($out, [$RowObject->name, $RowObject->email, $RowObject->amountParents, $RowObject->parentsNames, $RowObject->amountChildren, $RowObject->childrenNames, $RowObject->ageChildren, $RowObject->feier], ';');
				}
			}
			$inspirtation_result->close();
		}
...
//############################################################################################################################
	...
	private function get_feier()
	{
		return $this->feier;
	}

	private function set_feier($value)
	{
		$this->feier = $value;
	}

<?php 
    ...
    {
        include "admin/includes/locale/".$lang.".php";
    }
...
//        $message = "";
    ...
    function getUrlFriendlyText($string)
    {
        ...
        $validUrl  = preg_replace("/<br \/>/i","-",$validUrl);
    ...
    function sendHTMLImageMailTpl($imageName, $imagesDir, $message, $messageTemplate, &$te, $to, $from, $reply, $bcc, $subject, &$Subscriber)
    {
        $cid = md5(uniqid(rand(), 1));
        $mime_boundary = md5(uniqid(mt_rand(), 1));

        $fp = fopen ($imagesDir.$imageName, "rb");
        $str = fread ($fp, filesize($imagesDir.$imageName));
        $imageData = chunk_split(base64_encode($str));

        getLanguageLabels($te, 'de');
        $te->assign('salutation', $te->getVar("salutation".$Subscriber->gender."Label"));
        $te->assign('imageName', $imageName);
        $te->assign('emailMessage' , $message);

        $messageContent = "\n--".$mime_boundary."\n";
        $messageContent .= "Content-Type: text/html;charset=iso-8859-1\n";
        $messageContent .= "Content-Transfer-Encoding: 8bit\n\n";
        $messageContent .= $te->display($GLOBALS[$messageTemplate])."\n";
        $messageContent .= "\n--".$mime_boundary."\n";
        $messageContent .= "Content-Type: application/octetstream;\n\tname=".$imageName."\n";
        $messageContent .= "Content-Transfer-Encoding: base64\n";
        $messageContent .= "Content-ID: <".$imageName.">\n";
        $messageContent .= "Content-Disposition:inline;filename=".$imageName."\n\n";
        ...
        mail($to, $subject, $messageContent, $header, '-f noreply@pluspunkt.de');
    }
?>

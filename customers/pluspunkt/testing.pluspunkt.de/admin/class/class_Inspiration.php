...
##############################################################################################################################
	public function getInspirationToCSV($out)
	{
        ...
        fputcsv($out, array("Name", "Strasse", "PLZ / Stadt", "Telefon", "E-Mail", "Anzahl Erwachsene", "Anzahl Kinder", "Alter der Kinder"), ';');
		...
		if($inspirtation_result = $GLOBALS["db"]->query($inspirtation_query))
		{
			if($inspirtation_result->num_rows > 0)
			{
				while($RowObject = $inspirtation_result->fetch_object())
				{
                    fputcsv($out, array($RowObject->name, $RowObject->street, $RowObject->zipCodeCity, $RowObject->fon, $RowObject->email, $RowObject->amountParents, $RowObject->amountChildren, $RowObject->ageChildren), ';');
				}
			}
			$inspirtation_result->close();
		}

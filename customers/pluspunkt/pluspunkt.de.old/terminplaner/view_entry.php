...
// Most likely, the user just typed in a new event id in the URL.
...
if ( $can_edit && $event_status != "D" ) {
  ...
  $event_status != "D" )  {
  echo "<A HREF=\"del_entry.php?id=$id$u_url$rdate\" onClick=\"return confirm('" .
    translate("Are you sure you want to delete this entry?") .
...
if ( count ( $allmails ) > 0 ) {
  echo "<A HREF=\"mailto:" . implode ( ",", $allmails ) .
    "?subject=$subject\">" .

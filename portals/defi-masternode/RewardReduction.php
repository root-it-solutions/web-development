...
class RewardReduction
{
    ...
    public function getActualReward($block): string
    {
        ...
        if ($result = $GLOBALS["db"]->query($query))
        {
            ...
            {
                throw new Exception('Daten nicht gefunden');
            }
        }
        return '';
    }

    public function getActualRewardReductionInfo($block)
    {
        $query = 'SELECT * FROM reward_reduction_list WHERE block = (SELECT MAX(block) FROM reward_reduction_list WHERE block <= ' . $block . ')';
        if ($result = $GLOBALS["db"]->query($query))
        {
            if ($result->num_rows > 0)
            {
//                var_dump($result->fetch_object());exit;
